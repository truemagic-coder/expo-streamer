...
public class ExpoPlayAudioStreamModule: Module, AudioStreamManagerDelegate, MicrophoneDataDelegate, SoundPlayerDelegate {
    ...
    public func definition() -> ModuleDefinition {
        ...
        AsyncFunction("startRecording") { (options: [String: Any], promise: Promise) in
            ...
            // Safe access to audio session manager
            switch self.componentManager.getAudioSessionManager() {
            case .success(let manager):
                if let result = manager.startRecording(settings: settings, intervalMilliseconds: interval) {
        ...
        Function("pauseRecording") {
            if case .success(let manager) = self.componentManager.getAudioSessionManager() {
        ...
        AsyncFunction("stopRecording") { (promise: Promise) in
            switch self.componentManager.getAudioSessionManager() {
            case .success(let manager):
                if let recordingResult = manager.stopRecording() {
        ...
        AsyncFunction("clearPlaybackQueueByTurnId") { (turnId: String, promise: Promise) in
            switch self.componentManager.getAudioSessionManager() {
            case .success(let manager):
                manager.cleanPlaybackQueue(turnId, resolver: { _ in
                    promise.resolve(nil)
                }, rejecter: { code, message, error in
                    promise.reject(code ?? "ERR_UNKNOWN", message ?? "Unknown error")
                })
        ...
        AsyncFunction("pauseAudio") { (promise: Promise) in
            switch self.componentManager.getAudioSessionManager() {
            case .success(let manager):
                manager.pauseAudio(promise: promise)
            case .failure(let error):
                promise.reject("ERROR", error.localizedDescription)
            }
        }
